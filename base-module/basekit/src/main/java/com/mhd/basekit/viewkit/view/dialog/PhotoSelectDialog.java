...
public class PhotoSelectDialog extends Dialog implements OnClickListener, OnItemClickListener {
    ...
    public PhotoSelectDialog(Activity activity, int theme,
     ...
     * @param names          菜单项名称
     */
    public PhotoSelectDialog(Activity activity, int theme, SelectDialogListener listener, SelectDialogCancelListener cancelListener, List<String> names) {
        super(activity, theme);
        mActivity = activity;
        mListener = listener;
     ...
     * @param title    菜单标题文字
     */
    public PhotoSelectDialog(Activity activity, int theme, SelectDialogListener listener, List<String> names, String title) {
        super(activity, theme);
        mActivity = activity;
        mListener = listener;
        this.mName = names;
    ...
    public PhotoSelectDialog(Activity activity, int theme, SelectDialogListener listener, SelectDialogCancelListener cancelListener, List<String> names, String title) {
        super(activity, theme);
        mActivity = activity;
        mListener = listener;
        mCancelListener = cancelListener;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        View view = getLayoutInflater().inflate(R.layout.base_picture_dialog_select,
                null);
        setContentView(view, new LayoutParams(LayoutParams.FILL_PARENT,
                LayoutParams.WRAP_CONTENT));
        Window window = getWindow();
        // 设置显示动画
        window.setWindowAnimations(R.style.base_menu_animstyle);
    ...
    private class DialogAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            if (null == convertView) {
                ...
                convertView = layoutInflater.inflate(R.layout.base_picture_dialog_item, null);
            ...
            if (1 == mStrings.size()) {
                ...
                viewholder.dialogItemButton.setBackgroundResource(R.drawable.base_dialog_item_bg_only);
            } else if (position == 0) {
                viewholder.dialogItemButton.setTextColor(mFirstItemColor);
                viewholder.dialogItemButton.setBackgroundResource(R.drawable.base_select_dialog_item_bg_top);
            } else if (position == mStrings.size() - 1) {
                viewholder.dialogItemButton.setTextColor(mOtherItemColor);
                viewholder.dialogItemButton.setBackgroundResource(R.drawable.base_select_dialog_item_bg_buttom);
            } else {
                viewholder.dialogItemButton.setTextColor(mOtherItemColor);
                viewholder.dialogItemButton.setBackgroundResource(R.drawable.base_select_dialog_item_bg_center);
            }
            return convertView;
        }
    ...
    public static PhotoSelectDialog showDialog(Activity activity, PhotoSelectDialog.SelectDialogListener listener, SelectDialogCancelListener cancelListener) {
        List<String> names = new ArrayList<>();
        names.add("拍照");
        names.add("相册");
        PhotoSelectDialog dialog = new PhotoSelectDialog(activity, R.style
                .base_transparentFrameWindowStyle,
                listener, cancelListener, names);
