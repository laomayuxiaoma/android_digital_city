...
public class CalendarViewAdapter extends PagerAdapter {
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        ...
        if (calendarType == CalendarAttr.CalendayType.MONTH) {
            ...
            if (weekArrayType == 1) {
                calendar.showDate(CalendarUtils.getSaturday(current));
            } else {
                calendar.showDate(CalendarUtils.getSunday(current));
            }//每周的种子日期为这一周的最后一天
    ...
    public void setMarkData(HashMap<String, String> markData, HashMap<String, String> markData1) {
        CalendarUtils.setMarkData(markData,markData1);
    }
    ...
    public void switchToWeek(int rowIndex) {
        ...
        if (calendars != null && calendars.size() > 0 && calendarType != CalendarAttr.CalendayType.WEEK) {
            ...
            if (weekArrayType == 1) {
                v2.showDate(CalendarUtils.getSaturday(last));
            } else {
                v2.showDate(CalendarUtils.getSunday(last));
            }//每周的种子日期为这一周的最后一天
            ...
            if (weekArrayType == 1) {
                v3.showDate(CalendarUtils.getSaturday(next));
            } else {
                v3.showDate(CalendarUtils.getSunday(next));
            }//每周的种子日期为这一周的最后一天
