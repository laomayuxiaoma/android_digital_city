...
public class CalendarRenderer {
    private Week weeks[] = new Week[CalendarConst.TOTAL_ROW];    // 行数组，每个元素代表一行
    ...
    public void updateWeek(int rowIndex) {
        ...
        if (CalendarViewAdapter.weekArrayType == 1) {
            currentWeekLastDay = CalendarUtils.getSaturday(seedDate);
        } else {
            currentWeekLastDay = CalendarUtils.getSunday(seedDate);
        }
    ...
    private void instantiateMonth() {
        int lastMonthDays = CalendarUtils.getMonthDays(seedDate.year, seedDate.month - 1);    // 上个月的天数
        int currentMonthDays = CalendarUtils.getMonthDays(seedDate.year, seedDate.month);    // 当前月的天数
        int firstDayPosition = CalendarUtils.getFirstDayWeekPosition(
                seedDate.year,
                seedDate.month,
