...
public abstract class BaseMonthView extends BaseView {
    ...
    final void initMonthWithDate(int year, int month) {
        ...
        mHeight = mDelegate.getMonthViewShowMode() == CalendarViewDelegate.MODE_ALL_MONTH ? mItemHeight * mLineCount : CalendarUtil.getMonthViewHeight(year, month, mItemHeight, mDelegate.getWeekStart());
    ...
    private void initCalendar() {
        ...
        mCurrentItem = mItems.contains(mDelegate.getCurrentDay()) ? mItems.indexOf(mDelegate.getCurrentDay()) : mItems.indexOf(mDelegate.mSelectedCalendar);

        boolean listener = mDelegate.mCalendarInterceptListener != null;
        boolean selected = listener ? mDelegate.mCalendarInterceptListener.onCalendarIntercept(mDelegate.mSelectedCalendar) : false;
        if (mCurrentItem > 0 && listener && selected) {
            mCurrentItem = -1;
        }

        mLineCount = mDelegate.getMonthViewShowMode() == CalendarViewDelegate.MODE_ALL_MONTH ? 6 : (preDiff + monthDayCount + mNextDiff) / 7;
    ...
    final void updateWeekStart() {
        initCalendar();
        mHeight = mDelegate.getMonthViewShowMode() == CalendarViewDelegate.MODE_ALL_MONTH ? mItemHeight * mLineCount : CalendarUtil.getMonthViewHeight(mYear, mMonth, mItemHeight, mDelegate.getWeekStart());
    }
