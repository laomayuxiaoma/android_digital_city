...
final class LunarCalendar {
    ...
    private static String[] getSpecialFestivals(int year) {
        ...
        festivals[0] = startDiff == 7 ? dateToString(year, 5, startDiff + 1) + SPECIAL_FESTIVAL_STR[0] : dateToString(year, 5, startDiff + 7 + 1) + SPECIAL_FESTIVAL_STR[0];
        date.set(year, 5, 1);
        week = date.get(java.util.Calendar.DAY_OF_WEEK);
        startDiff = 7 - week + 1;
        festivals[1] = startDiff == 7 ? dateToString(year, 6, startDiff + 7 + 1) + SPECIAL_FESTIVAL_STR[1] : dateToString(year, 6, startDiff + 7 + 7 + 1) + SPECIAL_FESTIVAL_STR[1];

        date.set(year, 10, 1);
        week = date.get(java.util.Calendar.DAY_OF_WEEK);
        startDiff = 7 - week + 1;
        festivals[2] = startDiff <= 2 ? dateToString(year, 11, startDiff + 21 + 5) + SPECIAL_FESTIVAL_STR[2] : dateToString(year, 11, startDiff + 14 + 5) + SPECIAL_FESTIVAL_STR[2];
        return festivals;
    }
