...
public class BottomBarLayout extends LinearLayout implements ViewPager.OnPageChangeListener {
    ...
    private static final String STATE_MITEM = "save_item";


    private ViewPager mViewPager;
    private int mChildCount;//子条目个数
    private List<BottomBarItem> mItemViews = new ArrayList<>();
    private List<BottomBarItem> mAllItemViews = new ArrayList<>();
    private int mCurrentItem;//当前条目的索引
    private int mCurrentClickItem;//当前条目的索引
    ...
    private void init() {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            if (getChildAt(i) instanceof BottomBarItem) {
                ...
                mAllItemViews.add(bottomBarItem);
                //设置点击监听
    ...
    public void onPageSelected(int position) {
        if (getBottomItem(position).getmClickChange() == 0) {
            resetState();
    ...
    private class MyOnClickListener implements OnClickListener {
        ...
        public void onClick(View v) {
            //回调点击的位置
            if (mViewPager != null) {
                if (((BottomBarItem) v).getmClickChange() != 0) {
                    onItemSelectedListener.onItemSelected(getBottomItemFromAll(currentIndex), mCurrentClickItem, mCurrentClickItem);
                    return;
                }
                //有设置viewPager
                if (redressPosition(currentIndex) == mCurrentItem) {
                    //如果还是同个页签，使用setCurrentItem不会回调OnPageSelecte(),所以在此处需要回调点击监听
                    if (onItemSelectedListener != null) {
                        onItemSelectedListener.onItemSelected(getBottomItem(currentIndex), mCurrentItem, redressPosition(currentIndex));
                    }
                } else {
                    mViewPager.setCurrentItem(redressPosition(currentIndex), mSmoothScroll);
                }
            } else {
                //没有设置viewPager
                if (onItemSelectedListener != null) {
                    onItemSelectedListener.onItemSelected(getBottomItemFromAll(currentIndex), mCurrentItem, currentIndex);
                }
    ...
    private int redressPosition(int position) {
        int realPosition = position;
        for (int i = 0; i <= position; i++) {
            if (getBottomItemFromAll(i).getmClickChange() != 0) {
                realPosition -= 1;
            }
        }
        return realPosition;
    ...
    private void updateTabState(int position) {
        if (mItemViews.get(position).getmClickChange() == 0) {
            resetState();
    ...
    public BottomBarItem getBottomItemFromAll(int position) {
        return mAllItemViews.get(position);
    }
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        bundle.putInt(STATE_MITEM, mCurrentItem);
        return bundle;
    }
