...
//            city = new City(name, province, pinyin, code,0,0);
//            result.add(city);
//        }
//        cursor.close();
//        db.close();
...
//            String pinyin = cursor.getString(cursor.getColumnIndex(COLUMN_C_PINYIN));
//            String code = cursor.getString(cursor.getColumnIndex(COLUMN_C_CODE));
//            City city = new City(name, province, pinyin, code,0,0);
//            result.add(city);
//        }
//        cursor.close();
//        db.close();
...
//        return result;
        ...
        List<City> result = new ArrayList<>();
        List<City> cities = CityPicker.cities;
        int size = cities.size();
        for (int i = 0 ;i < size; i++){
            if (cities.get(i).getName().contains(keyword) || cities.get(i).getPinyin().contains(keyword)){
    ...
    public static City selecetCity(String name){
        List<City> cities = CityPicker.cities;
        int size = cities.size();
        for(int i = 0; i < size; i++){
            if (cities.get(i).getName().startsWith(name) || name.startsWith(cities.get(i).getName())){
                return cities.get(i);
            }
        }
        return null;
