...
public class ViewfinderView extends View {
    ...
    protected static final long ANIMATION_DELAY = 15L;
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (resultBitmap != null) {
            ...
            canvas.drawRect(frame.left - ScreenSpace, frame.top - ScreenSpace, frame.left + ScreenRate - ScreenSpace,
                    frame.top + CORNER_WIDTH - ScreenSpace, paint);
            canvas.drawRect(frame.left - ScreenSpace, frame.top - ScreenSpace, frame.left + CORNER_WIDTH - ScreenSpace, frame.top
                    + ScreenRate - ScreenSpace, paint);
            canvas.drawRect(frame.right - ScreenRate + ScreenSpace, frame.top - ScreenSpace, frame.right + ScreenSpace,
                    frame.top - ScreenSpace + CORNER_WIDTH, paint);
            canvas.drawRect(frame.right - CORNER_WIDTH + ScreenSpace, frame.top - ScreenSpace, frame.right + ScreenSpace, frame.top - ScreenSpace
                    + ScreenRate, paint);
            canvas.drawRect(frame.left - ScreenSpace, frame.bottom - CORNER_WIDTH + ScreenSpace, frame.left - ScreenSpace
                    + ScreenRate, frame.bottom + ScreenSpace, paint);
            canvas.drawRect(frame.left - ScreenSpace, frame.bottom + ScreenSpace - ScreenRate,
                    frame.left - ScreenSpace + CORNER_WIDTH, frame.bottom + ScreenSpace, paint);
            canvas.drawRect(frame.right + ScreenSpace - ScreenRate, frame.bottom + ScreenSpace - CORNER_WIDTH,
                    frame.right + ScreenSpace, frame.bottom + ScreenSpace, paint);
            canvas.drawRect(frame.right + ScreenSpace - CORNER_WIDTH, frame.bottom + ScreenSpace - ScreenRate,
                    frame.right + ScreenSpace, frame.bottom + ScreenSpace, paint);
...
//            paint.setAlpha(SCANNER_ALPHA[scannerAlpha]);
            ...
            if (middleLineTop <= 0){
                middleLineTop = frame.top;
            }
            if (mHeight <= 0) {
                mHeight = frame.height() / anmTime;
            }
            mDy = mHeight * (System.currentTimeMillis() - cTime);
            if (mDy < 1) {
                mDy = 1;
            }
            middleLineTop = middleLineTop + (int) mDy;
            if (middleLineTop > frame.top + frame.height() - 1) {
                middleLineTop = frame.top;
            }

            canvas.drawRect(frame.left + 2, middleLineTop - 1, frame.right - 1, middleLineTop + 4, paint);
...
//            paint.setAlpha(0x40);
    ...
    protected void onVisibilityChanged(@NonNull View changedView, int visibility) {
        ...
        Log.e("TTTTTTTTTTYY",visibility+"|||");
    }

    @Override
    protected void onWindowVisibilityChanged(int visibility) {
        super.onWindowVisibilityChanged(visibility);
        Log.e("TTTTTTTTTTYYY",visibility+"|||");
    }
